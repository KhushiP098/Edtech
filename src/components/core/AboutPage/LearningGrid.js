...
const LearningGrid = () => {
  ...
  return (
    <div className='grid grid-col-1 lg:grid-cols-4 mx-auto mb-10 p-5 lg:w-fit '>
        {
            data.map((card,index)=>{
                return(
                    <div key={index}
                     className={`${index===0 && "lg:col-span-2 lg:h-[280px] p-5" } ${card.order%2===1 ? "bg-richblack-700 lg-h-[250px] p-5" :"bg-richblack-800 lg-h-[250px] p-5"}
                      ${card.order===3  && "lg-col-start-2"} ${card.order<0 && "bg-transparent"}`} >

                      {
                        card.order<0 ?
                        (
                            <div className='lg:w-[90%] flex flex-col items-cente  pb-5 gap-3'>
                           ...
                           <div className='flex flex-col gap-8 p-7'>
                            ...
                            <p className='text-richblack-300 text-medium '> {card.description}</p>  
